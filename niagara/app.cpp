...
void renderApplication::mainLoop() {
    while (!glfwWindowShouldClose(window)) {
        ...
        char title[256];
        sprintf(title, "cpu: %.1f ms; gpu: %.3f ms; triangles %d; meshlets %d; RTX %s",
            frameCPUAvg, frameGPUAvg, meshes[0].m_indices.size() / 3, meshes[0].m_meshlets.size(), rtxEnabled ? "ON" : "OFF");
...
SwapChainSupportDetails renderApplication::querySwapChainSupport(VkPhysicalDevice device) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL renderApplication::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
